#include "deepstream_vehicle_count.h"

namespace VehicleCount {
  void
  DsVehicle::update_fps(gint id) {
  ...
  int
  DsVehicle::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  DsVehicle::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  DsVehicle::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
    ...
    txt_params->text_bg_clr.alpha = 1.0;
       ...
       line_params[0].x1 = 431;
       line_params[0].y1 = 477;
       line_params[0].x2 = 130; 
       line_params[0].y2 = 1900;  
       line_params[0].line_width = 4;
       line_params[0].line_color = (NvOSD_ColorParams){0.0, 0.1, 1.0, 1.0};
       display_meta->num_lines++;

        line_params[1].x1 = 843;//for demonstration, user need to se these values
       line_params[1].y1 = 1900;
       line_params[1].x2 = 567; 
       line_params[1].y2 = 482;
       line_params[1].line_width = 4;
       line_params[1].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
       display_meta->num_lines++;

        line_params[2].x1 = 431;//for demonstration, user need to se these values
       line_params[2].y1 = 477;
       line_params[2].x2 = 567;
       line_params[2].y2 = 482; 
       line_params[2].line_width = 4;
       line_params[2].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
       display_meta->num_lines++;
       
        line_params[3].x1 = 130;//for demonstration, user need to se these values
       line_params[3].y1 = 1900;
       line_params[3].x2 = 843;
       line_params[3].y2 = 1900;    
       line_params[3].line_width = 4;
       line_params[3].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
       display_meta->num_lines++;
  ...
  GstPadProbeReturn
  DsVehicle::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      ...
      int inside_roi = cv::pointPolygonTest(data, cv::Point2f(cur_x, cur_y), false);
  ...
  gboolean
  DsVehicle::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  DsVehicle::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  DsVehicle::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  DsVehicle::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  DsVehicle::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  DsVehicle::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  DsVehicle::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        VehicleCount::DsVehicle::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", VehicleCount::DsVehicle::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!VehicleCount::DsVehicle::set_tracker_properties(nvtracker)) {
  ...
  void DsVehicle::setPaths(guint num_sources) {

    // Config Paths
    PGIE_YOLO_DETECTOR_CONFIG_FILE_PATH =
    strdup("models/BusModel/config_infer_primary_yolov5s.txt");
...
int main(int argc, char *argv[]) {
  ...
  VehicleCount::DsVehicle DsVehicle;
  GMainLoop *loop = NULL;
  ...
  ctx = g_option_context_new("DsVehicle DeepStream App");
  group = g_option_group_new("DsVehicle", NULL, NULL, NULL, NULL);
  g_option_group_add_entries(group, DsVehicle.entries);
  ...
  gint sources = DsVehicle.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  DsVehicle.setPaths(num_sources);
  ...
  int fail_safe = DsVehicle.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, DsVehicle.bus_call, loop);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      DsVehicle.tiler_src_pad_buffer_probe, NULL, NULL);
  }
